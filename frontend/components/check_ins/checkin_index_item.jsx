...
class CheckinIndexItem extends React.Component {
  ...
  render() {
    ...
    const showDelete =
      checkIn.author_id === sessionId ? "show-delete" : "hide-delete";
    let drunks_arr = [];
        ...
        <div className="checkin-item-row-1">
          <div>
            <img
              ...
              src={window[`user${userImgId}`]}
              alt=""
            />
          </div>
          {/* <div><img className="profile-pic" src={checkIn.author.img_url}alt={window.profile} /></div> */}
          <div className="text">
            <p className="author-name">{checkIn.authorName} </p> is drowning in
            an
            <Link to={`/profiles/${checkIn.author}`}></Link>
            <Link to={`/drinks/${checkIn.drinkId}`}>
              {" "}
              {checkIn.drinkName}
            </Link>{" "}
            by <Link to={`/bars/${checkIn.barId}`}>{checkIn.barName}</Link>
          </div>
        </div>
        <div className="checkin-item-row-2">
          <div className="checkin-item-tri"></div>
          <div className="checkin-body">
            <p className="author-name">{checkIn.authorName}</p> says:{" "}
            <p>{checkIn.body}</p>
            <div className="checkin-body-bottom">
              <img
                className="check-rating-img"
                src={drunks}
                width="30%"
                height="auto"
              />
            ...
            <button className="like-button" onClick={this.props.likeCheck}>
              Like
            </button>
          </div>
        </div>
